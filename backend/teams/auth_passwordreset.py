...
SENDER_EMAIL_PASSWORD = "COMP3900" 
PORT = 465
...
def auth_password_reset_request(email):
    ...
    if existing_reset_code is None:
        ...
        existing_reset_code.reset_code = new_reset_code
...
def auth_password_reset(password_reset_code, new_password):
    ...
    if reset_code is not None:
        ...
        user = db.session.query(User).filter_by(id=reset_code.user_id).first()
        user.hash_password(new_password)
        
        # Delete the password reset code as it has now been used
        db.session.query(ResetCode).filter_by(user_id=reset_code.user_id).delete()
